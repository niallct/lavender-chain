This is a very basic introduction to those new to the game, written to complement the rules for indoor and pairs games. Those familiar with the game should just read the list of rule changes.

Where we introduce some slightly more advanced aspects, they are ((like this)). It's OK to ignore those for the first few times you play.

## Core concept
There are:
*  two people batting, who try to score runs / points
*  one person bowling, who tries to prevent the batters making runs, and to get them out
*  others fielding, who assist the bowler. In the normal outdoor game the bowler and fielders are all on one side. 

In the normal `village' outdoor game, one side bats and the other bowls. Then they swap. Whoever scores most runs, wins. 

## The indoor `pairs' format
In the indoor sessions, we play a variation called `pairs'. Players are put into pairs, then one pair bats, another takes turns to bowl, and the others field. 
Pairs rotate around so that everyone does the same amount of each.

The batting pair score points by making runs, similar to the traditional game. Two big differences in pairs:
*  Batters keep batting, when they are out but lose points
*  You can also score points as a bowler or fielder by getting someone out.

The session facilitators will either help to field, or apply the rules (umpire) and keep track of the scores (scorer).
...
-  stand roughly side-on, in front of the wicket, head turned to face the bowler
-  if you are right handed point your left shoulder towards the bowler and have your right hand lower down on the bat than your left. Swap if you are left-handed.
-  leave enough space behind you that you don't hit the wicket with your bat
-  but not so far forward to be in front of the line if you miss the ball
-  ((watch where the ball is going, and where it bounces, and try to move in response to that.))

### The batting pair not facing the ball should:
-  be ready to run
-  stand on the other side of the wicket to the bowler
- ((watch where the ball goes if the striker can't see it and decide whether to run))

In indoor pairs, there are no runs from hitting the ball behind you, only in front. For a ball hit in front, the batter who hit it should say ‘yes’ or ‘no’ to inform their partner whether they want to run or not.
...
-  ask if they are not sure about something
-  respect each others' ability and stage of learning the game
...
We can play pairs outdoors too, with the same idea that everyone rotates round all roles. Of course there are no walls, so:
-  the boundary, counting four or six, is a rope or white line around the edge of the field
-  the distance between the wickets is larger, just over 20 metres.
-  we almost always use a hard ball, so batters and fielders wear protective equipment
-  there is no restriction on running if you hit the ball behind you
-  there is no side wall, and each completed run scores one, not two
-  a batter is out if the ball hits their body (usually a leg or boot) on the way to the wicket (and some other conditions are met)
-  an illegal ball has only one run penalty, but has to be bowled again
